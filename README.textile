...
The Contact page includes a "contact request" form. When the visitor submits the form, an email message goes to the site owner containing the contents of the form. The tutorial shows how to set up the application so email can be sent using the site owner's Gmail account. The contact form can be adapted for other purposes, such as an appointment request.
...
<pre>
question  One starter app is available for Rails 4.0. More to come.
      1)  learn-rails
</pre>
...
<pre>
LearnRails::Application.config.secret_key_base = '...some really long, random string...'
</pre>
...
Submitting the form on the "Contact" page should send an email mesage to the site owner's address.
